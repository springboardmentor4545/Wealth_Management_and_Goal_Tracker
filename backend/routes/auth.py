...
from routes.security import verify_password, create_access_token, hash_password
...
def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    cur.execute(
        "SELECT id, name, email, risk_profile, risk_score, kyc_status, profile_completed, created_at FROM users WHERE email = %s",
        (email,)
    )
    user = cur.fetchone()
